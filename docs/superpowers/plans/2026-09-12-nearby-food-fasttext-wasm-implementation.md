...
- `apps/web/src/features/nearby-food/fasttext-runtime/`：存放由 Vite 处理的官方 WASM 运行时源码和二进制。
- `apps/web/public/models/nearby-classifier/`：存放 Web 端按 URL 读取的量化模型。
...
- [x] **Step 3: Implement the smallest protocol module**
...
- [x] **Step 5: Commit the protocol boundary**
...
- [x] **Step 3: Implement the injected adapter and one-time model promise**
...
- [x] **Step 4: Add the production loader for static browser assets**
...
const loadModelFromBrowserAssets: FastTextLoader = async () => {
  const runtime = await import('./fasttext-runtime/fasttext.js') as {
...
- [x] **Step 5: Run adapter tests and lint**
...
**Files:**
- Create: `apps/web/src/features/nearby-food/fasttext-runtime/fasttext.js`
- Create: `apps/web/src/features/nearby-food/fasttext-runtime/fasttext_wasm.js`
- Create: `apps/web/src/features/nearby-food/fasttext-runtime/fasttext_wasm.wasm`
...
- [x] **Step 1: Build the official fastText WASM runtime outside the repository**
...
- [x] **Step 2: Copy the runtime into Vite source assets and the model into Web public assets**

```bash
mkdir -p apps/web/src/features/nearby-food/fasttext-runtime
mkdir -p apps/web/public/models/nearby-classifier
cp /private/tmp/lets-eat-fasttext-src/webassembly/fasttext.js apps/web/src/features/nearby-food/fasttext-runtime/fasttext.js
cp /private/tmp/lets-eat-fasttext-src/webassembly/fasttext_wasm.js apps/web/src/features/nearby-food/fasttext-runtime/fasttext_wasm.js
cp /private/tmp/lets-eat-fasttext-src/webassembly/fasttext_wasm.wasm apps/web/src/features/nearby-food/fasttext-runtime/fasttext_wasm.wasm
...
The runtime is kept under `src` so Vite can rewrite the generated `new URL('fasttext_wasm.wasm', import.meta.url)` reference. Do not import files from `public` as JavaScript modules. Do not copy the `.bin` model, training datasets, Emscripten SDK, or native fastText executable into Web assets.

- [x] **Step 3: Verify the runtime and model paths**

Run: `test -s apps/web/src/features/nearby-food/fasttext-runtime/fasttext.js && test -s apps/web/src/features/nearby-food/fasttext-runtime/fasttext_wasm.js && test -s apps/web/src/features/nearby-food/fasttext-runtime/fasttext_wasm.wasm && test -s apps/web/public/models/nearby-classifier/nearby-restaurant-classifier.ftz`

Expected: all four files exist and are non-empty.

- [x] **Step 4: Commit the model/runtime assets**

```bash
git add apps/web/src/features/nearby-food/fasttext-runtime apps/web/public/models/nearby-classifier
...
- [x] **Step 2: Run the focused hook test to verify it fails**

Run: `pnpm --filter @lets-eat/web test -- useNearbyRestaurantClassifications.test.ts`

Expected: FAIL because the hook does not exist.

- [x] **Step 3: Implement the hook with synchronous fallback and cancellable async updates**
...
- [x] **Step 4: Wire the hook into `NearbyFoodPage`**
...
- [x] **Step 5: Add page-level fallback-to-model coverage**
...
- [x] **Step 6: Run the Web test suite and lint**
...
- [x] **Step 7: Commit the page integration**
...
- [x] **Step 1: Build the Web application**

Run: `pnpm --filter @lets-eat/web build`

Expected: Vite succeeds and the generated `dist` contains the model under `models/nearby-classifier/` plus hashed runtime JavaScript/WASM assets under `dist/assets/`.

- [x] **Step 2: Start the local stack and exercise the nearby list**
...
The local Amap search requires a configured browser key and a location provider, so the page-level integration test covers the list state with an injected classifier; the real browser check exercises the generated runtime and trained model directly.

- [x] **Step 3: Inspect the browser network and console output**

Confirm the browser requests one generated fastText runtime chunk, one generated `.wasm` asset, and one `.ftz` model file for a page session, with no request for the `.bin` model or training data.
...
- [x] **Step 5: Review the final diff and commit any documentation-only adjustment**

Run: `git status --short` and `git diff --stat`.

Only task files, the Vite-managed runtime assets, and the quantized model may be included in the implementation commits. Existing unrelated modifications must remain unstaged.
