...
export function RoomPage({ roomClient, userId, room, onRoomChange }: RoomPageProps) {
  ...
  const closeOrLeave = async () => {
    if (!window.confirm(isHost ? '确定关闭房间吗？' : '确定退出房间吗？')) return;
    setBusy(true);
    try {
      ...
      <section className="space-y-4">
        ...
        {isHost ? (
          <>
            ...
            <button type="button" disabled={busy} onClick={() => void roomClient.startRound(room).then((round) => navigate(`/game/round/${round.id}`))} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-[#FFD100] px-4 py-4 font-black shadow-lg"><Play className="h-5 w-5" />开始游戏</button>
          </>
        ) : (
