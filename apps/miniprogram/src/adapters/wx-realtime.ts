...
export function createWxRealtimeTransport(
  ...
  const handleMessage = (raw: unknown) => {
    ...
    if (authenticated) handleEvent(message);
  };

  const open = () => {
    if (stopped || !options) return;
    const nextSocket = wx.connectSocket({ url: getWebSocketUrl(baseUrl) }) as unknown as SocketLike;
    socket = nextSocket;
    authenticated = false;
    nextSocket.onOpen(() => {
      if (stopped || !options) return;
    ...
    nextSocket.onError(() => nextSocket.close({ code: 1011, reason: 'socket error' }));
  };
